...
public class Percolation {
    ...
    private int[][] grid;                   // The indices of each site in the system, modeled as an x/y plane.
    private WeightedQuickUnionUF unionFind; // The union-find network that will control which sites are in which components.
...
//            int[] adjacentSites = adjacentIds(adjacentSites(row - 1, col - 1));
            ...
            for (int[] site : adjacentSites) {
                ...
                if (siteOpen[adj]) {
                    boolean sitePercolation = full[unionFind.find(id)];
                    boolean adjPercolation = full[unionFind.find(adj)];

                    unionFind.union(id, adj);

                    idComp = unionFind.find(id);
                    full[idComp] = sitePercolation || adjPercolation;
                    if (row == grid.length || site[0] + 1 == grid.length) {
                        percolates = full[idComp];
                    }
                }
            }
        }
    ...
    /**
     * For each provided site, retrieves the corresponding site ID.
     * @param sites An array of [row, column] coordinates
     * @return An array of site IDs
     */
    private int[] adjacentIds(int[][] sites) {
        int[] ids = new int[sites.length];
        for (int i = 0; i < sites.length; i++) {
            ids[i] = grid[sites[i][0]][sites[i][1]];
        }
        return ids;
    }
    ...
    private int[][] adjacentSites(int row, int col) {
        ...
        for (int i = 0; i < abstractAdjacents.length; i++) {
            if (!withinBounds(abstractAdjacents[i][0])) {
                adjLength--;
                validCoordinates[i] = false;
            }

            if (!withinBounds(abstractAdjacents[i][1])) {
                adjLength--;
                validCoordinates[i] = false;
            }
        }
     ...
     * @return true if the provided integer is within the bounds of N
     */
    private boolean withinBounds(int c) {
